...
static wpi::expected<constrained_solvepnp::RobotStateMat, slp::ExitStatus>
newton_solve(constrained_solvepnp::RobotStateMat x_guess,
             std::vector<ProblemState<3>*>& cameras) {
  using FullStateMat = typename ProblemState<3>::FullStateMat;
  using StateMat = typename ProblemState<3>::StateMat;
  using HessianMat = typename ProblemState<3>::HessianMat;
  using GradMat = typename ProblemState<3>::GradientMat;
  ...
  for (int iter = 0; iter < 100; iter++) {
    ...
    for (alpha_refinement = 0; alpha_refinement < 100; alpha_refinement++) {
      ...
      casadi_real new_cost = sumJ(trial_x);
...
constrained_solvepnp::do_optimization_multi(
  ...
  states.reserve(nCameras);
  std::vector<ProblemState<3>*> ptrs;
  ptrs.reserve(nCameras);
  ...
  for (int i = 0; i < nCameras; i++) {
    ...
    auto field = field2points[i];
    auto obs = point_observations[i];
    if (field.cols() != (tags * 4) || obs.cols() != (tags * 4)) {
      return wpi::unexpected{
          slp::ExitStatus::NONFINITE_INITIAL_COST_OR_CONSTRAINTS};
    }

    for (int c = 0; c < obs.cols(); ++c) {
      obs(0, c) = (obs(0, c) - cameraCals[i].cx) / cameraCals[i].fx;
      obs(1, c) = (obs(1, c) - cameraCals[i].cy) / cameraCals[i].fy;
    }

    int usedTags = tags > 10 ? 10 : tags;
    if (usedTags != tags) {
      std::cerr << "constrained_solvepnp: camera has " << tags
                << " tags; using first 10\n";
      field = field.leftCols(usedTags * 4);
      obs = obs.leftCols(usedTags * 4);
    }

    auto problemOpt = createProblem(usedTags, heading_free);
    if (!problemOpt) {
      return wpi::unexpected{
          slp::ExitStatus::NONFINITE_INITIAL_COST_OR_CONSTRAINTS};
    }

    states.push_back(ProblemState<3>{robot2cameras[i], field, obs,
                                     cameraCals[i], *problemOpt, gyroθ,
                                     gyroErrorScaleFac});
  }
  ...
  return newton_solve(x_guess, ptrs);
}
